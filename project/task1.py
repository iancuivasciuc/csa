...
memory = np.zeros(MEMORY_SIZE, 'uint16')
files = np.zeros((MAX_FD, 2), 'uint16')


def add(fd, dim, fout):
    left, right = 0, 0
    while right < MEMORY_SIZE:
        if memory[right] != 0:
            left = right + 1
        elif memory[left] == 0 and right - left + 1 == dim:
            memory[left:right + 1] = fd
            files[fd] = [left, right]
            break

        right += 1

    fout.write(f"{fd}: ({files[fd][0]}, {files[fd][1]})\n")


def get(fd, fout):
    fout.write(f"({files[fd, 0]}, {files[fd, 1]})\n")
...
def main():
    if len(sys.argv) != 2:
        print("Usage: python3 task1.py <test.in>")
        return
